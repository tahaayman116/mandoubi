...
function RepresentativeForm() {
  const { userProfile, addSubmission } = useFirebaseAuth();
  ...
  // حساب عدد غير المستلمين تلقائياً
  const calculateNotReceived = () => {
    const total = parseInt(formData.totalPeople) || 0;
    const received = parseInt(formData.receivedMoney) || 0;
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate form
    const totalPeople = parseInt(formData.totalPeople) || 0;
    const received = parseInt(formData.receivedMoney) || 0;

    if (received > totalPeople) {
      alert('عدد المستلمين لا يمكن أن يكون أكبر من العدد الكلي');
      setLoading(false);
      return;
    }
    ...
    try {
      const submissionData = {
        ...formData,
        notReceived: calculateNotReceived(),
      ...
      <div className="max-w-2xl mx-auto relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 text-lg">
            أدخل بيانات القرية والمستفيدين
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label htmlFor="villageName" className="block text-sm font-semibold text-gray-700 mb-2">
              اسم القرية *
            </label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="receivedMoney" className="block text-sm font-medium text-gray-700 mb-2">
                عدد المستلمين *
              </label>
              <input
                id="receivedMoney"
            ...
            <div>
              <label htmlFor="notReceived" className="block text-sm font-medium text-gray-700 mb-2">
                عدد غير المستلمين (محسوب تلقائياً)
              </label>
              <input
                id="notReceived"
                name="notReceived"
